        ...
        if (transaction) {
          ...
          for (let n = 0; n < transaction.length; n++) {
            if (transaction[n].PbKey_Trade_Login_Type == '8') {
              _this.CID = transaction[n].PbKey_Trade_Login_CID;
            }
          }
        } else {
          alert('交易未登录！')
    ...
    methods: {
      ...
      conNoneClick: function (conditionID, flag1, flag3, i) {
        ...
        setTimeout(() => {
          ...
          _this.operateTip(flag1, flag3, '止盈止损');
        }, 360)
      },
...
//                                    },1000)
      ...
      nativeInteraction: function () {
        if (pbPage.getInitState()) {
          ...
          pbPage.addReloadFunByKey('stopM', this.pageReload);
        } else {
          pbPage.initPage({
            callbacks: [
...
//                  if(p[2] && p[2].length != 0){
//                    _this.noneTouchLen[n].createTime = p[2];
//                  }
                }
                break;
              }
