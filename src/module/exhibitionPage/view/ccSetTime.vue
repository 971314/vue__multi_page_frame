    ...
    <div class="time-selected-items">
      <div class="time-selected-options">
        ...
        <div class="time-start-content">
          <span>{{ccSetTime}}</span>
          <input type="date" v-model="ccSetTime"/>
        </div>
      </div>
    </div>
...
<script>
  export default {
    ...
    activated () {
      this.ccSetTime = this.$$timeFormate({date: this.$$getCurrentTime(), format: 'Y-M-D'})
    },
    mounted () {
    ...
    methods: {
      //保存时间
      parseParams () {
        this.$store.dispatch('updateCcSetTime',this.ccSetTime)
      }
    }
  }
</script>
